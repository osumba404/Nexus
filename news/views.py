...
def _user_key(request) -> str:
    """Unique cache key per authenticated user or per browser session."""
...
def _get_user_defaults(request):
    if request.user.is_authenticated and hasattr(request.user, 'profile'):
        ...
        return {'default_country': p.default_country or 'ke', 'default_categories': p.get_categories_list()}
...
def _saved_states(request, articles: list) -> dict:
    ...
    qs = SavedArticle.objects.filter(user=request.user, url__in=urls).values('url', 'interaction_type')
    states: dict = {}
    for row in qs:
...
def news_feed(request):
    """HTMX-only endpoint — returns feed partial."""
    params = _get_filter_params(request)
    result = NewsAggregatorService.get_news(
        **{k: v for k, v in params.items()},
        user_key=_user_key(request),
...
def my_feed(request):
    ...
    trending_articles = NewsAggregatorService.get_trending(country=params.get('country') or 'ke')
    ctx = _base_context(request, params, result)
    ctx['trending']   = trending_articles
    ctx['is_my_feed'] = True
    return render(request, 'news/home.html', ctx)
...
def toggle_interaction(request):
    """Toggle like or bookmark for an article. Works via HTMX."""
    interaction_type = request.POST.get('type')   # 'like' or 'bookmark'
    ...
    obj, created = SavedArticle.objects.get_or_create(
        ...
        defaults=dict(title=title, excerpt=excerpt, image_url=image_url,
    ...
    # Return the updated button fragment
...
def dashboard(request):
    ...
    articles = SavedArticle.objects.filter(
        user=request.user, interaction_type=interaction
    ).order_by('-saved_at')
    return render(request, 'news/dashboard.html', {
        'saved_articles': articles,
        'tab':            tab,
        'bookmark_count': SavedArticle.objects.filter(user=request.user, interaction_type=SavedArticle.BOOKMARK).count(),
        'like_count':     SavedArticle.objects.filter(user=request.user, interaction_type=SavedArticle.LIKE).count(),
    })
